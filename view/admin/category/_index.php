...
<body>
    ...
    <main class="ease-soft-in-out xl:ml-68.5 relative h-full max-h-screen rounded-xl transition-all duration-200 ps ps--active-y">
        <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex flex-row justify-between">
            <h6 class="text-lg uppercase font-semibold">Categories</h6>
            <a href="<?= url('admin.category.create') ?>" class=" mt-2 mr-32 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 border border-green-700 rounder">Create</a>
        </div>
            ...
            <tbody>
                <?php foreach ($categories as $key => $value) { ?>
                    <tr>
                        ...
                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                            <span class="font-semibold leading-tight text-sm text-black"><?= $value['name_category'] ?></span>
                        </td>
                        <td class="pl-48 p-2 leading-normal text-center align-middle bg-transparent border-b text-sm whitespace-nowrap shadow-transparent">
                            <img src="<?= asset($value['img_category']) ?>" alt="" class="w-10">
                        </td>
                        ...
                        <td class="p-2 flex items-center justify-center">
                            <form action="<?= url('admin.category.edit') ?>" method="get">
                            ...
                            <form action="<?= url('admin.category.delete') ?>" method="get">
                                <input type="hidden" name="id" value="<?= $value['id'] ?>">
